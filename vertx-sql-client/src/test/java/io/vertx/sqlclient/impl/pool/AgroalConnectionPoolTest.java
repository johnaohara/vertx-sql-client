...
public class AgroalConnectionPoolTest {
  ...
  public void testSimple() {
    ...
    AgroalConnectionPool pool = new AgroalConnectionPool(queue, 1);
  ...
  public void testReleaseConnectionWhenWaiterQueueIsEmpty() {
    ConnectionQueue queue = new ConnectionQueue();
    AgroalConnectionPool pool = new AgroalConnectionPool(queue, 2);
  ...
  public void testMaxQueueSize1() {
    ConnectionQueue queue = new ConnectionQueue();
    AgroalConnectionPool pool = new AgroalConnectionPool(queue, 1, 0);
    SimpleHolder holder1 = new SimpleHolder();
    pool.acquire(holder1);
    SimpleConnection conn = new SimpleConnection();
    queue.connect(conn);
  ...
  public void testAcquireOnlyConnectOnce() {
    ConnectionQueue queue = new ConnectionQueue();
    AgroalConnectionPool pool = new AgroalConnectionPool(queue, 10, 0);
    SimpleHolder holder1 = new SimpleHolder();
    pool.acquire(holder1);
    assertEquals(1, queue.size());
  }
